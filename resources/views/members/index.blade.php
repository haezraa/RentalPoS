    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 h-fit">
            ...
            <form action="{{ route('members.store') }}" method="POST">
                ...
                <button type="submit" class="w-full bg-[#2251a5] text-white py-3 rounded-xl font-bold hover:bg-blue-800 transition">
                    Simpan Data
                </button>
            </form>
        </div>
        ...
        <div class="md:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($members as $member)
                        <tr class="hover:bg-blue-50/50">
                            ...
                            <td class="px-6 py-4 text-right">
                                <form action="{{ route('members.destroy', $member->id) }}" method="POST" onsubmit="return confirm('Hapus member ini?')">
                                    @csrf @method('DELETE')
                    ...
                    @empty
                        <tr><td colspan="4" class="px-6 py-8 text-center text-gray-400 italic">Belum ada member.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
